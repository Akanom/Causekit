...
class CrossFitter:
    ...
    def __init__(
        self,
        *,
        propensity_factory: NuisanceFactory,
        outcome_factory: NuisanceFactory,
        n_splits: int = 5,
    ...
    def fit_predict(self, X: Any, *, treatment: Any, outcome: Any) -> CrossFitResult:
        ...
        for arm in (0.0, 1.0):
            positions = np.flatnonzero(assigned.to_numpy() == arm)
            rng.shuffle(positions)
            folds[positions] = np.arange(len(positions)) % self.n_splits
