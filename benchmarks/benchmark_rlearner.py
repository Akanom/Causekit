"""One-run honest R-learner CATE comparison on verified NSW data.

Every row uses the same CauseKit-native outcome and propensity nuisances, immutable honest
roles, outer folds, evaluation metrics, and max-t draws. Optional scikit-learn regressors
replace only the weighted CATE learner, so the comparison does not conflate nuisance and
CATE differences and never creates a CauseKit runtime dependency.
...
SEED = 20_260_729
DATASET = "nsw_mixtape"
DESIGN_ID = "nsw_job_training_honest_cate_v1"
COVARIATES = (
    "age",
    "educ",
    "black",
...
MODEL_NAMES = (
    ...
    "sklearn_weighted_ridge_cv",
    "sklearn_weighted_hist_gradient_boosting",
    "sklearn_weighted_random_forest",
)
...
class _StandardizedRidgeCV:
    def fit(
        ...
        *,
        ...
        self.model_ = RidgeCV(alphas=np.logspace(-6, 4, 6)).fit(
            transformed,
            y,
...
def _sklearn_cate_factory(name: str) -> tuple[Callable[[], Any], str]:
    try:
        import sklearn
    ...
    if name == "sklearn_weighted_ridge_cv":
        factory: Callable[[], Any] = _StandardizedRidgeCV
    elif name == "sklearn_weighted_hist_gradient_boosting":
    ...
    elif name == "sklearn_weighted_random_forest":
        ...
        raise ValueError(f"Unknown R-learner comparator {name!r}.")
    return factory, sklearn.__version__


def _estimator(name: str) -> tuple[RLearner, str | None]:
    if name == "causekit_native_weighted_ridge_gcv":
        cate_factory = None
        comparator_version = None
    elif name == "causekit_native_spline_ridge_gcv":
        cate_factory = NativeSplineRidgeCATE
        comparator_version = None
    else:
        cate_factory, comparator_version = _sklearn_cate_factory(name)
    return (
        RLearner(
...
def _run_once(name: str, design: dict[str, Any]) -> dict[str, Any]:
    ...
    tracemalloc.stop()
    heterogeneity = result.calibration_coefficients["heterogeneity"]
    return {
        "model": name,
        "status": "completed",
        ...
        "calibration_heterogeneity": heterogeneity,
        ...
        "cate_basis_family": _cate_diagnostic(result, "basis_family"),
        "cate_selected_knot_count": _cate_diagnostic(result, "selected_knot_count"),
        "cate_basis_dimension": _cate_diagnostic(result, "basis_dimension"),
    }
...
def main() -> None:
    ...
    report = {
        ...
        "comparison_policy": (
            ...
            "only the weighted CATE learner changes"
        ),
